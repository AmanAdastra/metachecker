...
#time_format = datetime.now().strftime("%Y-%m-%d %S")
#logger = copy.deepcopy(logger)
separator = "\n------------------------------------------------------------------------------------------------------\n"
fmt = "{level} | {time:ddd MMMM YYYY, HH:mm:ss:SSS} | {name}:{function}:{line}" + separator + "{message}" + separator
logger.remove()
logger = copy.deepcopy(logger)     
...
logger.add("./logs/admin_logs/admin_module.log", colorize=True, rotation=rotator.should_rotate, level=constants.LOG_LEVEL, format=fmt, enqueue=True, backtrace=True)
